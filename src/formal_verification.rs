...
mod kani_proofs {
    use crate::ntt::{
        ct_butterfly_lazy_t_step, ct_butterfly_step, fused_norm_step, fused_step_kernel,
        gs_butterfly_lazy_step, T_OFFSET_LAZY_T,
    };
    ...
    // section and `soak_differential_vs_pqclean` (host-tests/tests/soak.rs).
    ...
    fn fused_norm_per_iter_safe() {
        ...
        let max_per_iter: u64 =
            2 * ((Q as u64 / 2) * (Q as u64 / 2) + 2047u64 * 2047);
    ...
    /// validate() accepts a prepared pubkey when every slot holds a coefficient
    /// in `[0, Q)`. Symbolically: an arbitrary in-range coefficient at an
    /// arbitrary position, with all other slots zero. Since `validate()`
    /// inspects slots independently, this generalises to whole-array validity.
    #[kani::proof]
    fn validate_accepts_valid() {
        ...
        let pk = crate::Falcon512PreparedPubkey::from_bytes(bytes);
        assert!(pk.validate(), "validate should accept any in-range coefficient");
    }
    ...
    // =========================================================================
    //
    // Refinement claim, symbolic over the full per-coefficient input space:
    ...
    //     invariants intact.
    //
    // Together: distinct valid coefficients ⇒ distinct canonical bytes ⇒
    // decoder recovers each one ⇒ byte-level malleability is precluded for
    // the per-coefficient layer. The two-coefficient lift is
    // `decompress_two_coeffs_matches_spec` below (symbolic concatenation);
    // full N=512 lift is the proptest suite
    // (`canonicality_under_bit_flips` and `decompress_signature_matches_lean_encode_all_spec`).
    ...
    fn decompress_one_coeff_matches_spec() {
        ...
        let result = crate::codec::decompress_one_coeff(
            &bytes,
            &mut acc,
            &mut acc_len,
            &mut idx_in,
    ...
    fn decompress_two_coeffs_matches_spec() {
        ...
        kani::assume(
            (9 + (m0 as usize) / 128 + 9 + (m1 as usize) / 128) <= 7 * 8,
        );
        ...
        let mut acc_len: u64 = 0;
        let mut idx_in: usize = 0;
        let r0 = crate::codec::decompress_one_coeff(
            &bytes, &mut acc, &mut acc_len, &mut idx_in,
        );
        let r1 = crate::codec::decompress_one_coeff(
            &bytes, &mut acc, &mut acc_len, &mut idx_in,
        );
        ...
        assert!(r1 == Some(exp1), "second coefficient mismatch (concatenation broken)");

        // Bit-stream invariants after two coefficients.
        let bytes_consumed = (total_bits + 7) / 8;
        assert!(idx_in == bytes_consumed, "idx_in advance mismatch after pair");
        assert!(acc_len <= 7, "acc_len exceeds bit-stream invariant");
    }
}
...
mod proptest_formal {
    ...
    //
    // This proptest is the operational refinement bridge: we emit those
    ...
    // and demand exact recovery. Together with the per-coefficient Kani
    // harness `decompress_one_coeff_matches_spec`, this closes the
    // sequence-level Rust↔Lean refinement.
    // -------------------------------------------------------------------
    ...
    fn encode_coeff_spec_bits(sign: bool, mag: u16, out: &mut Vec<bool>) {
        ...
        for _ in 0..high { out.push(false); }
        out.push(true);
    }
    ...
    /// 625*8 = 5000, so the bias keeps `Σ mag/128` well under ~390.
    ...
    proptest! {
        ...
        /// **Spec-refinement.** Build canonical bytes for a random N=512
