...
-- Rust optimization rests on, and the full byte-level codec canonicality.
...
-- composition across the whole array — is verified operationally (Rust
-- kernel-vs-spec proptests + 1M PQClean differential), not in Lean.
...
--   4. Full byte-level codec canonicality (`serializeFalcon_injective`):
...
import Falcon512.Canonicality    -- Full byte-level codec canonicality (`serializeFalcon_injective`)
